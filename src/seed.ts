...
const seed = async () => {
  ...
  for (const category of categories) {
    // 1) Always ensure parent category is correct (find by slug, then update or create)
    ...
    if (existingParent.docs.length > 0) {
      const parent = existingParent.docs[0];

      // Force-correct: top-level parent with right name/color
    ...
    // 2) For each subcategory, force it under the right parent (update or create)
    for (const subCategory of category.subcategories || []) {
      ...
      if (existingSub.docs.length > 0) {
        ...
        // Force-correct: right name + right parent
